...
test('should edit expense from firebase', (done) => {
    ...
    const expense = { description: 'new Gum', amount: 30000};
    const store = createMockStore({});
    store.dispatch(startEditExpense(id, expense) ).then(() => {
        ...
        database.ref(`expenses/${id}`)
            ...
            .then((snapshot) => {
                ...
                expect(updatedExpense.description).toBe(expense.description);
                expect(updatedExpense.amount).toBe(expense.amount);
                done();
            });
    })
});
